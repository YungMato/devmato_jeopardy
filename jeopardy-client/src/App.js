...
const avatarMap = Object.fromEntries(
  avatarOptions.map(a => [a.key, a.url])
);
                  ...
                  <div className="question-cell" key={qId}>
                    <button
                      className={
                        ...
                        (isDisabled && !isAnswered ? " question-button--disabled" : "")
                      }
...
const resetLobby = () => {
        ...
        <header className="app-header">
          ...
          <div className="app-status">
            <span className="badge-connection">
              <span className={"badge-dot " + (connected ? "badge-dot--ok" : "badge-dot--bad")} />
              {connected ? "Verbunden" : "Nicht verbunden"}
            </span>
            {me && <span style={{ fontSize: 12 }}>Logged in als {me.name}</span>}
          </div>
        </header>

        {error && <p className="error-text">Fehler: {error}</p>}

        <p style={{ fontSize: 12 }}>Lobby-Code: {lobby.code}</p>
  ...
  <div className="panel panel--primary" style={{ marginTop: 12 }}>
    <div className="panel-header">
      <h2>Lobby</h2>
      <span className="panel-subtitle">Teams bilden & Gamemaster wählen</span>
    </div>
  ...
  <div className="board-info-bar">
    ...
    <span className="board-info-title">
      {
        lobby.boards.find(b => b.id === lobby.selectedBoardId)?.title
        || "Unbekanntes Board"
      }
    </span>
  </div>
  ...
  {me.isGamemaster && lobby.boards && (
    <div className="board-select">
      <span style={{ fontSize: 12, marginRight: 6 }}>Spielfeld:</span>
      <select
        className="input"
        style={{ minWidth: 160 }}
  ...
  .map((p) => (
    <span key={p.id} className="lobby-player-pill">
      {p.name}
      {p.isGamemaster && <span className="tag tag--gm" style={{ marginLeft: 4 }}>GM</span>}
    </span>
))}
    ...
    <div className="lobby-teams-section">
      ...
      <div className="teams-grid-lobby">
        {[0, 1, 2].map((slotIndex) => {
          ...
          const teamId = teamIds[slotIndex];
          const team = teamId ? lobby.teams[teamId] : null;
          ...
          const avatarOption = avatarOptions.find(a => a.key === team.avatarKey);
          ...
          return (
            <div className="team-slot" key={team.id}>
              <div
  className={
    "team-card" + (me.teamId === team.id ? " team-card--mine" : "")
  }
>
  ...
  onClick={() => {
    if (me.teamId === team.id) {
      setOpenAvatarTeamId(openAvatarTeamId === team.id ? null : team.id);
    }
  }}
  style={{ cursor: me.teamId === team.id ? 'pointer' : 'default' }}
>
  ...
  {team.playerIds.map((pid) => {
    ...
    return (
      <li
        ...
        className={
          "team-member-name" +
          (pid === recentlyJoinedPlayerId ? " team-member-join" : "")
        }
      >
        {p.name}
        {p.id === me.id && (
...
{openAvatarTeamId && (
  <div className="avatar-picker-overlay" onClick={() => setOpenAvatarTeamId(null)}>
    <div
